...
resource "google_compute_instance_group" "myig" {
  name        = "${var.stone_name}-terraform-webservers"
...
resource "google_compute_address" "default" {
  name = "${var.stone_name}-website-ip-1"
  region = "${var.region}"
...
resource "google_compute_forwarding_rule" "default" {
 # provider = google-beta
  ...
  name   = "${var.stone_name}-website-forwarding-rule"
  region = "${var.region}"
  ip_protocol           = "HTTP"
...
resource "google_compute_region_target_http_proxy" "default" {
  #provider = google-beta
  region  = "${var.region}"
  name    = "${var.stone_name}-website-proxy"
...
resource "google_compute_region_url_map" "default" {
  #provider = google-beta
  region          = "${var.region}"
  name            = "${var.stone_name}-website-map"
...
resource "google_compute_region_backend_service" "default" {
  ...
  region      = "${var.region}"
  name        = "${var.stone_name}-website-backend"
  protocol    = "HTTP"
  timeout_sec = 10
...
resource "google_compute_region_health_check" "default" {
 ...
 # provider = google-beta
  region = "${var.region}"
  name   = "${var.stone_name}-website-hc"
  http_health_check {
